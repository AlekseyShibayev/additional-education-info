...
1. Простые:
  ...
  - добавить в Chat boolean переключалку, слушать бота или нет
  - создать сущность "Подписка", сейчас есть подписка вб и курса, связать с Chat через @ManyToMany
  - вынести информацию о клиенте в отдельную сущность, например там будет поле Role, связать с Chat через @OneToOne
  - добавить возможность клиенту подписываться на бота, например через сообщение в него (пусть для начала это будет "+" и "-")
  ...
  - добавить @Validate
  - добавить тестов морды (MockMvc) и jpa
